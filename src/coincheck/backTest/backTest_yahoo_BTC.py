...
# plotModeSql = "select ticker, ticker from minutes_results where sheet_name ~ '" + sheet_name + "' order by diff_per desc"
plotModeSql = "select ticker, company_name from ticker t where ticker = 'BTC-JPY'"
...
witch_bare_data = "yahoo"


# witch_bare_data = "dynamo"
    ...
    def get_ticker_list(self):
        try:
            ...
            if mode == "plotMode":
                ...
                select_sql = plotModeSql  # minutesデバッグ用
    ...
    def loop_check(self, ticker_list):
        ...
        for idx, tickers in enumerate(ticker_list, 0):
            try:
                ...
                bare_data = None
                if witch_bare_data == "yahoo":
                    bare_data = y_data(tickers[0])
                elif witch_bare_data == "dynamo":
...
def get_results(bare_data, ticker, company_name):
    ...
    if witch_bare_data == "yahoo":
        ...
        y_list = [d['buy'] for d in bare_data]
    ...
    if db_name == 'minutes_results':
        # 株式市場の場合、15:00_Closeから～9:00_Openまでの間があるので、時間をシフトして連続データということにする
    ...
    if mode == "plotMode":
        if db_name == 'minutes_results':
            ...
            plot(x_list, y_list_mean_s, y_list_mean_l, x_labels, y_list, sell_x_list, buy_x_list, ticker,
                 company_name, rsi_y_list)
        else:
...
def plot(x_list, y_list_mean_s, y_list_mean_l, x_labels, y_list, sell_x_list, buy_x_list, ticker, company_name,
         rsi_y_list):
    plt.figure()
    plt.grid(True)
